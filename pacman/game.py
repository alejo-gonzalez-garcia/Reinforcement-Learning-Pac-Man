...
class Game(object):
    ...
    def run(self):
        ...
        if self.isLoggingEnabled(): 
            """Create log file if BasicAgentAA or KyeboardAgent is used"""
        ...
        while not self.gameOver:
            ...
            if self.isLoggingEnabled() and agentIndex == 0: 
                logFile.write(f"{agent.printLineData(self.state)}")
        ...
        """Close log file."""
        if self.isLoggingEnabled(): logFile.close()
    ...
    def isLoggingEnabled(self):
        ...
        return(isinstance(self.agents[0], BustersAgent))

    def createLogFile(self) :
        # Create log file
        outputPath = "./logs"
        fileName = "all_data_pacman"
        extension = ".arff"
        file = f"{fileName}{extension}"
        if not os.path.exists(outputPath): 
            os.makedirs(outputPath)

        logFile = open(os.path.join(outputPath, file), "a")
       
        # Write file header
        if os.path.getsize(f"{outputPath}/{file}") == 0: 
            logFile.write(f"{self.agents[0].printLineHeader(fileName)}")
